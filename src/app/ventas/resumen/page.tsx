...
export default async function ResumenVentasPage() {
  ...
  const [{ data: oportunidades }, { data: responsables }, { data: tiposCliente }] = await Promise.all([
    supabase
      ...
      .select(
        'id, estado, fecha_ingreso, monto_estimado, comision_monto, comision_liquidada, responsable_id, crm_prospectos(tipo_cliente_id, crm_tipos_cliente(nombre)), perfiles(nombre_completo)'
      )
